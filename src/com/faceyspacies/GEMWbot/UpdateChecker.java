...
/**
 * The UpdateChecker checks to see if the price on the wiki for an item is the same as the price in
 * the API for an item. If it isn't, begin GrandExchangeUpdater.
 * 
 * @author Ty
 *
 */
public class UpdateChecker extends BaseWikiTask {
   ...
   * 
   * @param itemName This is actually the entire page name. So Module:Exchange/itemName should be
   *        passed.
  ...
  protected void Start() {
    while (running) {
      try {
        ...
        timestamp = null;
        String jagexPrice = loadCurPrice("1319").getPrice();
        String wikiPrice = loadPriceFromWiki("Module:Exchange/Rune 2h sword");
